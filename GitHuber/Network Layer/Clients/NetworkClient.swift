...
final class NetworkClient {
    ...
    private let networkQueue = DispatchQueue(label: "com.gitHuber.networkqueue",
                                             attributes: .concurrent)
    private let storage = TokenStorage(storage: KeyChainStorage.standard)
}
...
private extension NetworkClient {
    ...
    func decodeData<Response>(request: Request<Response>,
        ...
        guard let decode = try? JSONDecoder().decode(Response.self, from: data) else {
            if let decodedError = try? JSONDecoder().decode(GitlabAuthError.self, from: data) {
                completion(.failure(NetworkErrors.gitlabAuthError(decodedError)))
            } else {
